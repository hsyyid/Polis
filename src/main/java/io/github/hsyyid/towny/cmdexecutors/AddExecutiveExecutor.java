				...
				if (ConfigManager.getLeader(townName).equals(player.getUniqueId().toString()))
				{
					try
					{
						if (ConfigManager.getMembers(townName).contains(p.getUniqueId().toString()))
						{
							...
							player.sendMessage(Texts.of(TextColors.GREEN, "[Towny]: ", TextColors.YELLOW, "Added " + p.getName() + " as an exeuctive of town " + townName));
						}
						else
						{
					...
					{
						player.sendMessage(Texts.of(TextColors.GREEN, "[Towny]: ", TextColors.DARK_RED, "Error! ", TextColors.RED, "Failed to add town executive!"));
					}
				}
				else
				{
